...
def print_event(evt):
    ...
    if root.findall(".//{0}li[@class='ios-signalstate-ev']".format(namespace)):
         print("\tState : " + root.find(".//{0}li[@class='ios-signalstate-ev']/{0}span".format(namespace)).text)
...
class RWPanel:
     ...
     def subscribe(self):       
         ...
         # payload = "resources=1&1=/rw/rapid/symbol/data/RAPID/T_ROB1/uimsg/PNum;value&1-p=1"
         payload = {'resources':['1','2','3'],
             '1':'/rw/panel/speedratio',
             '1-p':'1',
             '2':'/rw/iosystem/signals/di0;state',
             '2-p':'2',
             '3':'/rw/iosystem/signals/di1;state',
             '3-p':'2'}        
