...
class ExecuteSubscriber:
    ...
    def close(self):
        self.ws.close()
        if len(ws.execute_data) != 0:
            save_df = pd.DataFrame(ws.execute_data, columns=["A_X", "A_Y", "A_Z", "B_X", "B_Y", "B_Z", "C_X", "C_Y", "C_Z", "Time"])
            save_df.to_csv(ws.execute_file, header=True, index=False)
